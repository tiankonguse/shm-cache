...
public:
	LinkTable() :
			header(NULL), blocks(NULL) {
		errmsg[0] = 0;
	}
	~LinkTable() {
	}

	ull64_t EvalBufferSize(ull64_t uBlockCount, ull64_t uBlockSize) {
		return sizeof(LinkTableHead) + (uBlockCount * uBlockSize);
	}
	ull64_t EvalBlockCount(ull64_t uBuffSize, ull64_t uBlockSize) {
		return (uBuffSize - sizeof(LinkTableHead)) / uBlockSize;
	}

	//初始化链式表，用户必须自己分配LinkTableHead和Block所需的buffer
	//可以调用EvalBufferSize根据块大小和块数计算链表需要的buffer大小
	int Init(void *pBuffer, ull64_t ddwBufferSize, ull64_t ddwBlockCount,
			ull64_t ddwBlockSize);
	int InitExisting(void *pBuffer, ull64_t ddwBufferSize,
	...
	int GetUsage() {
		return header ?
				(100
						- ((header->ddwFreeBlockCount * 100)
								/ header->ddwAllBlockCount)) :
				0;
	}
