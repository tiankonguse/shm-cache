...
// if you don't have one, we won't blame it, you can download it
#ifdef HAS_ATTR_API
#include "Attr_API.h"
#else
#define Attr_API(a, b)
...
int LinkTable::Init(void *pBuffer, ull64_t ddwBufferSize, ull64_t ddwBlockCount,
		ull64_t ddwBlockSize) {
	if (pBuffer == NULL
			|| ddwBufferSize < EvalBufferSize(ddwBlockCount, ddwBlockSize)) {
...
int LinkTable::InitExisting(void *pBuffer, ull64_t ddwBufferSize,
		ull64_t ddwBlockCount, ull64_t ddwBlockSize) {
	if (pBuffer == NULL
			|| ddwBufferSize < EvalBufferSize(ddwBlockCount, ddwBlockSize)) {
		snprintf(errmsg, sizeof(errmsg), "Bad argument: %s",
				pBuffer == NULL ? "buffer is null" : "not enough buffer size");
	...
	if (header->ddwAllBlockCount != (ddwBlockCount - 1)) {
		snprintf(errmsg, sizeof(errmsg),
				"Total block count mismatched: existing %llu, expected %llu",
				header->ddwAllBlockCount, (ddwBlockCount - 1));
		return -2;
	}

	if (header->ddwBlockSize != ddwBlockSize) {
		snprintf(errmsg, sizeof(errmsg),
				"Block size mismatched: existing %llu, expected %llu",
				header->ddwBlockSize, ddwBlockSize);
		return -3;
	}
...
int LinkTable::FreeBlock(ull64_t ddwStartPos) {
	...
	if ((ddwStartPos == 0) || (ddwStartPos >= header->ddwAllBlockCount)) {
		...
		snprintf(errmsg, sizeof(errmsg), "Position %llu gone insane",
				ddwStartPos);
		return -2;
	}
	...
	if (ddwNextPos != 0) {
		...
		snprintf(errmsg, sizeof(errmsg), "Too many blocks at %llu",
				ddwStartPos);
		return -4;
	}
...
int LinkTable::GetEmptyBlock(int iCount, ull64_t &ddwOutPos) {
	...
	Attr_API(355999, 1);
#endif
	if ((iCount < 0) || (iCount > MAX_BLOCK_COUNT)
	...
	i = 0;
	while (i < iCount && ddwCurPos > 0 && ddwCurPos < header->ddwAllBlockCount
...
int LinkTable::GetData(ull64_t ddwPos, void *sDataBuf, int &iOutDataLen) {
	...
	if (sDataBuf == NULL || iBufLen < (int) BLOCK_DATA_LEN()) // buffer大小不足一个块
	{
...
int LinkTable::SetData(ull64_t &ddwPos, const void *sDataBuf, int iDataLen) {
	...
	if (iCount > MAX_BLOCK_COUNT) {
		Attr_API(356019, 1);
		snprintf(errmsg, sizeof(errmsg),
				"Data too large to fit in max of %d blocks", MAX_BLOCK_COUNT);
		return -2;
	}
	...
	while ((ddwNextPos) && (iLeftLen > 0)) {
		...
		if (iLeftLen > (int) BLOCK_DATA_LEN()) {
			memcpy(pBlock->bufData, ((char*) sDataBuf) + iCopyLen,
			BLOCK_DATA_LEN());
...
int LinkTable::PrintInfo(const string &prefix) {
	...
	cout << prefix << "ddwFreeBlockCount: " << header->ddwFreeBlockCount
			<< endl;
	cout << prefix << "ddwBlockSize:      " << header->ddwBlockSize << endl;
	cout << prefix << "ddwFirstFreePos:     " << header->ddwFirstFreePos
			<< endl;
	cout << prefix << "ddwRecycleIndex:     " << header->ddwRecycleIndex
			<< endl;
