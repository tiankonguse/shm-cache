 ...
 * 构造函数
 */
shm_adapter::shm_adapter(uint64_t ddwShmKey, uint64_t ddwShmSize, bool bCreate,
		bool bReadOnly, bool bLock, bool bHugePage, bool bInitClear) {
	int iRet;
	m_bIsNewShm = false;
	...
	if (iShmId < 0) {
		//连接失败, 并且是非创建的, 返回失败
		if (!(nFlag & IPC_CREAT)) {
			m_strErrMsg = "shmget without create failed - ";
		...
		if (iShmId < 0) {
			m_strErrMsg = "shmget with create failed - ";
			m_strErrMsg += strerror(errno);
			return;
		}
		//Attach
		...
		if (m_pShm == (void *) -1) {
			m_strErrMsg = "shmat with create failed - ";
			m_strErrMsg += strerror(errno);
			m_pShm = NULL;
			return;
		}
		...
		m_pShm = shmat(iShmId, NULL, 0);
		if (m_pShm == (void *) -1) {
			m_strErrMsg = "shmat with exist shm failed - ";
			m_strErrMsg += strerror(errno);
			m_pShm = NULL;
			return;
		}
...
bool shm_adapter::open(uint64_t ddwShmKey, bool bReadOnly) {
	...
	int iRet = shmctl(nShmId, IPC_STAT, &stShmStat);
	if (iRet < 0) {
		m_strErrMsg = "shmctl to get size failed - ";
		m_strErrMsg += strerror(errno);
		return false;
	}
	//Attach共享内存
 ...
 * 创建共享内存
 */
bool shm_adapter::create(uint64_t ddwShmKey, uint64_t ddwShmSize,
		bool bHugePage, bool bInitClear) {
	if (m_pShm) {
		m_strErrMsg = "Shm is NOT null, you have opened one already";
		return false;
	}
	...
	if (nShmId < 0) {
		...
		int iRet = shmctl(nShmId, IPC_STAT, &stShmStat);
		if (iRet < 0) {
			m_strErrMsg = "shmctl with size failed - ";
			m_strErrMsg += strerror(errno);
			return false;
		}
		//大小不一致
